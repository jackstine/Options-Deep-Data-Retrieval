...
from src.repos.equities.companies.company_repository_mock import create_company_repository_mock
from src.repos.equities.tickers.ticker_repository_mock import create_ticker_repository_mock
from src.repos.equities.tickers.ticker_history_repository_mock import create_ticker_history_repository_mock
...
class TestCompanyPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_ticker_history_repo = create_ticker_history_repository_mock(seed=12345)
    ...
    def test_run_ingestion_with_single_source(self):
        ...
        self.assertGreater(len(all_companies), 3)  # More than initial 3 predefined companies
    ...
    def test_run_ingestion_with_multiple_sources(self):
        ...
        # Verify results
        self.assertGreater(results["inserted"], 5)  # Should have companies from both sources
        self.assertEqual(results["errors"], 0)
        
        # Verify companies from both sources were processed
        all_companies = self.mock_company_repo.get_all_companies()
        company_sources = [c.source for c in all_companies if hasattr(c, 'source') and c.source]
    ...
    def test_identify_new_companies(self):
        ...
        companies = [
            ...
            Company(
                ...
                exchange="NASDAQ",
                ticker=Ticker(symbol="AAPL", company_id=None)  # AAPL exists in mock repo
            )
        ]
        
        existing_symbols = self.mock_company_repo.get_active_company_symbols()
        new_companies = self.pipeline._identify_new_companies(companies, existing_symbols)
    ...
    def test_identify_companies_to_update(self):
        ...
        existing_symbols = self.mock_company_repo.get_active_company_symbols()
        companies_to_update = self.pipeline._identify_companies_to_update(companies, existing_symbols)
    ...
    def test_create_ticker_histories_for_companies(self):
        ...
        ticker_histories = self.pipeline._create_ticker_histories_for_companies(companies)
    ...
    def test_get_unused_tickers(self):
        ...
        screener_symbols = {"ACTIVE1", "AAPL", "MSFT"}  # Include existing symbols from mock repo
    ...
    def test_pipeline_database_operations_integration(self):
        ...
        self.mock_ticker_history_repo = create_ticker_history_repository_mock(seed=99999)
        ...
        ticker_histories_in_db = self.mock_ticker_history_repo.get_all_ticker_histories()
