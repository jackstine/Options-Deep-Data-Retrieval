"""Integration tests for High/Low backfill pipeline.
...
from tests.integration.algorithms.high_lows.backfill_test_helpers import (
...
class TestHighLowBackfillPipeline:
    """Integration tests for high/low backfill pipeline with split adjustment and multiple configs."""
    ...
    def test_backfill_with_split_and_multiple_configs(self):
        ...
        with integration_test_container("options-deep-test-data:latest") as (
            ...
            from src.repos.algorithms.high_lows.rebounds_repository import (
                ReboundsRepository,
            )
            ...
            lows_repo = LowsRepository()
            rebounds_repo = ReboundsRepository()
            ...
            configs = [
                HighLowPatternConfig(threshold=threshold)
            ...
            testsplit_lows = lows_repo.get_by_ticker_history_id(testsplit_th.id)
            testdelist_lows = lows_repo.get_by_ticker_history_id(testdelist_th.id)
            testactive_lows = lows_repo.get_by_ticker_history_id(testactive_th.id)

            print(f"\n📊 TESTSPLIT: {len(testsplit_lows)} lows in database")
            print(f"📊 TESTDELIST: {len(testdelist_lows)} lows in database")
            print(f"📊 TESTACTIVE: {len(testactive_lows)} lows in database")

            # Validate TESTSPLIT patterns in database with exact assertions
            validate_patterns_in_database(
                patterns=testsplit_lows,
                ticker_symbol="TESTSPLIT",
                ticker_history_id=testsplit_th.id,
    ...
    def test_backfill_single_ticker_with_split(self):
        ...
        with integration_test_container("options-deep-test-data:latest") as (
            ...
            configs = [HighLowPatternConfig(threshold=single_threshold)]
            ...
            lows = lows_repo.get_by_ticker_history_id(testsplit_th.id)
            validate_patterns_in_database(
                patterns=lows,
                ticker_symbol="TESTSPLIT",
                ticker_history_id=testsplit_th.id,
            ...
            print(f"📊 Generated {len(lows)} low patterns for TESTSPLIT")
    ...
    def test_backfill_delisted_company(self):
        ...
        with integration_test_container("options-deep-test-data:latest") as (
            ...
            # Total patterns generated (active + completed) - TESTDELIST 15% = 2 total (no completed)
            expected_total_count = expected_db_count
            ...
            # Verify patterns in database with exact count and threshold
            lows = lows_repo.get_by_ticker_history_id(testdelist_th.id)
            validate_patterns_in_database(
                patterns=lows,
                ticker_symbol="TESTDELIST",
                ticker_history_id=testdelist_th.id,
            ...
            print(f"📊 Generated {len(lows)} low patterns for TESTDELIST")
