"""Helper functions for low/high backfill integration tests."""
...
class FixtureDataExpectations:
    ...
    # Active patterns in database (highs table)
    TESTSPLIT_PATTERNS = {
        Decimal("0.10"): 2,
        Decimal("0.15"): 2,
        Decimal("0.20"): 2,
    ...
    TESTSPLIT_DB_COUNT = 7  # Active patterns in DB
    TESTSPLIT_TOTAL = 7  # Total generated (active + completed)
    ...
    TESTDELIST_DB_COUNT = 4  # Active patterns in DB
    TESTDELIST_TOTAL = 6  # Total generated (active + completed)
    ...
    TESTACTIVE_DB_COUNT = 4  # Active patterns in DB
    TESTACTIVE_TOTAL = 6  # Total generated (active + completed)
...
def validate_patterns_in_database(
    ...
    for pattern in patterns:
        ...
        assert pattern.low_start_date is not None, "Pattern should have low_start_date"
        assert pattern.low_start_price is not None, "Pattern should have low_start_price"
        # high_threshold_price is optional - only set when pattern crosses threshold
...
def validate_pattern_dates_before_delisting(
    ...
    """
    for pattern in patterns:
        if pattern.low_start_date:
            assert pattern.low_start_date <= delisting_date, (
                f"{ticker_symbol} pattern date {pattern.low_start_date} "
