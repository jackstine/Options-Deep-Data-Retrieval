...
class CompanyPipeline:
    ...
    def run_comprehensive_sync(self, sources: List[CompanyDataSource]) -> Dict[str, int]:
        """
    ...
    def _comprehensive_sync_to_database(self, companies: List[Company]) -> Dict[str, int]:
        """
        ...
        try:
            ...
            new_companies = self._identify_new_companies(companies, existing_company_symbols)
            companies_to_update = self._identify_companies_to_update(companies, existing_company_symbols)
                ...
                if companies_inserted > 0:
                    ...
                    if inserted_companies_with_ids:
                        ...
                        if tickers_to_insert:
                            ...
                            results["tickers_inserted"] = self.ticker_repo.bulk_insert_tickers(tickers_to_insert)
                        ...
                        if ticker_histories_to_insert:
                            ...
                            results["ticker_histories_inserted"] = self.ticker_history_repo.bulk_insert_ticker_histories(ticker_histories_to_insert)
            ...
            if companies_to_update:
                ...
                for company in companies_to_update:
                    ...
                    try:
                        ...
                        self.logger.error(f"Error updating company {ticker_symbol}: {e}")
                        results["errors"] += 1
            
            # 3. Handle companies with new ticker symbols (existing companies, new tickers)
            new_ticker_companies = self._identify_new_tickers(companies, existing_ticker_symbols)
            ...
            if existing_companies_with_new_tickers:
                ...
                if additional_tickers:
                    ...
                    additional_tickers_inserted = self.ticker_repo.bulk_insert_tickers(additional_tickers)
    ...
    def _identify_new_companies(self, companies: List[Company], existing_symbols: Set[str]) -> List[Company]:
    ...
    def _identify_companies_to_update(self, companies: List[Company], existing_symbols: Set[str]) -> List[Company]:
        ...
        self.logger.info(f"Identified {len(companies_to_update)} companies for potential updates")
        return companies_to_update
    
    def _identify_new_tickers(self, companies: List[Company], existing_ticker_symbols: Set[str]) -> List[Company]:
    ...
    def _create_tickers_for_companies(self, companies: List[Company]) -> List[TickerDataModel]:
    ...
    def _create_ticker_histories_for_companies(self, companies: List[Company]) -> List[TickerHistoryDataModel]:
