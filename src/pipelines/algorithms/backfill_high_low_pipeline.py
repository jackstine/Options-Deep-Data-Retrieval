"""Pipeline for backfilling high/low patterns from historical data."""
...
from src.models.low import Low
from src.repos.algorithms.lows_repository import LowsRepository
from src.repos.algorithms.rebounds_repository import ReboundsRepository
...
class BackfillResult(TypedDict):
    ...
    active_lows_inserted: int
    rebounds_inserted: int
    errors: int


class BackfillHighLowPipeline:
    """Pipeline for backfilling high/low patterns from historical pricing data.
    ...
    5. Bulk inserts resulting lows and rebounds into database

    This is a one-time operation for generating historical pattern data.
    After backfill, use DailyHighLowPipeline for incremental updates.
    """
    ...
    def run(
        ...
        """Backfill high/low patterns for a ticker.
        ...
        try:
            ...
            for threshold in thresholds:
                ...
                try:
                    ...
                    # Collect results
                    all_active_lows.extend(processed.active_lows)
                    all_rebounds.extend(processed.completed_rebounds)

                    self.logger.info(
                        f"Threshold {float(threshold):.0%}: "
                        f"{len(processed.active_lows)} active patterns, "
                        f"{len(processed.completed_rebounds)} completed rebounds"
                    )
            ...
            if all_active_lows:
                self.logger.info(f"Inserting {len(all_active_lows)} active low patterns")
                upsert_result = self.lows_repo.bulk_upsert_lows(all_active_lows)
                result["active_lows_inserted"] = (
                    upsert_result["inserted"] + upsert_result["updated"]
                )

            if all_rebounds:
                self.logger.info(f"Inserting {len(all_rebounds)} completed rebounds")
                rebounds_inserted = self.rebounds_repo.bulk_insert_rebounds(
                    all_rebounds
                )
                result["rebounds_inserted"] = rebounds_inserted

            result["total_patterns_generated"] = len(all_active_lows) + len(
                all_rebounds
            )
