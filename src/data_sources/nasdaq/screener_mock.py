...
class NasdaqScreenerLoaderMock:
    ...
    def load_file(self, file_path: str | Path) -> List[Company]:
        ...
        self.logger.info(f"Mock: Successfully loaded {len(companies)} companies from screener file")
        return companies
    ...
    def load_directory(self, directory_path: str | Path) -> List[Company]:
        ...
        directory_path = Path(directory_path) if directory_path else Path("mock_directory")
        ...
        self.logger.info(f"Mock: Found {len(mock_files)} screener files in {directory_path}")
        
        for filename in mock_files:
            try:
                # Generate different number of companies per file for realism
                file_company_count = self.fake.random_int(20, 100)
                companies = [self._create_fake_company() for _ in range(file_company_count)]
                all_companies.extend(companies)
                self.logger.info(f"Mock: Loaded {len(companies)} companies from {filename}")
            except Exception as e:
                self.logger.error(f"Mock: Error loading file {filename}: {e}")
                continue
        
        self.logger.info(f"Mock: Total companies loaded from all screener files: {len(all_companies)}")
...
class NasdaqScreenerSourceMock(CompanyDataSource):
    ...
    def __init__(self, screener_dir: Optional[str] = None, seed: int = 12345):
        """
...
def create_nasdaq_screener_source_mock(screener_dir: Optional[str] = None, 
                                     seed: int = 12345) -> NasdaqScreenerSourceMock:
...
class ScreenerFunctionsMock:
    ...
    def load_screener_files_from_directory(self, directory_path: str | Path) -> List[Company]:
...
def mock_load_screener_files_from_directory(directory_path: str | Path) -> List[Company]:
