...
class NasdaqScreenerLoader:
    ...
    def load_file(self, file_path: str | Path) -> List[Company]:
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded {len(companies)} companies from screener file")
            return companies
    ...
    def load_directory(self, directory_path: str | Path) -> List[Company]:
        ...
        for file_path in screener_files:
            try:
                ...
                self.logger.info(f"Loaded {len(companies)} companies from {file_path.name}")
            except Exception as e:
                self.logger.error(f"Error loading file {file_path.name}: {e}")
                continue
        
        self.logger.info(f"Total companies loaded from all screener files: {len(all_companies)}")
        return all_companies
    ...
    def _convert_row_to_company(self, row: dict) -> Optional[Company]:
        """
