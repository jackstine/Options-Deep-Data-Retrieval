...
class YahooFinanceProviderMock(DataSourceBase):
    ...
    def _initialize_supported_symbols(self) -> None:
        ...
        major_symbols = [
            ...
            "NFLX", "UBER", "LYFT", "ZOOM", "SHOP", "SQ", "PYPL", "CRM",
            "ORCL", "IBM", "INTC", "CSCO", "BA", "JPM", "BAC", "WFC",
            "V", "MA", "JNJ", "PFE", "KO", "PEP", "WMT", "TGT"
        ]
    ...
    def _generate_mock_stock_quote(self, symbol: str) -> Optional[StockQuote]:
        """
        ...
        try:
            ...
            day_high = base_price + self.fake.pyfloat(min_value=0, max_value=price_variance)
            day_low = base_price - self.fake.pyfloat(min_value=0, max_value=price_variance)
            
            # Previous close should be close to current price
            previous_close = base_price + self.fake.pyfloat(min_value=-price_variance*0.5, max_value=price_variance*0.5)
            
            # Open price typically near previous close
            open_price = previous_close + self.fake.pyfloat(min_value=-price_variance*0.3, max_value=price_variance*0.3)
            
            # 52-week range should be wider
            week_52_high = base_price + self.fake.pyfloat(min_value=price_variance, max_value=price_variance*3)
            week_52_low = base_price - self.fake.pyfloat(min_value=price_variance, max_value=price_variance*2)
            ...
            quote = StockQuote(
                ...
                pe_ratio=Decimal(str(round(self.fake.pyfloat(min_value=5, max_value=50), 2))),
                dividend_yield=Decimal(str(round(self.fake.pyfloat(min_value=0, max_value=8), 2))) if self.fake.boolean(chance_of_getting_true=60) else None,
    ...
    def generate_batch_quotes(self, symbol_count: int = 10) -> List[StockQuote]:
        ...
        selected_symbols = self.fake.random_choices(available_symbols, length=symbol_count)
...
def create_yahoo_finance_provider_mock(timeout: int = 30, seed: int = 12345) -> YahooFinanceProviderMock:
...
if __name__ == "__main__":
    ...
    for quote in quotes[:2]:
        ...
        print(f"  52-Week Range: ${quote.fifty_two_week_low} - ${quote.fifty_two_week_high}")
        if quote.pe_ratio:
