...
class CompanyRepositoryMock:
    ...
    def get(self, filter_model: Optional[CompanyDataModel] = None) -> List[CompanyDataModel]:
        ...
        for company in companies:
            ...
            if filter_model.id is not None and company.id != filter_model.id:
                match = False
            if filter_model.active is not None and company.active != filter_model.active:
                match = False
            if (filter_model.sector is not None and filter_model.sector != "" 
                and company.sector != filter_model.sector):
                match = False
            if (filter_model.exchange is not None and filter_model.exchange != ""
                and company.exchange != filter_model.exchange):
                match = False
            if (hasattr(filter_model, 'ticker') and filter_model.ticker is not None
                and company.ticker and company.ticker.symbol != filter_model.ticker):
                match = False
    ...
    def get_one(self, filter_model: CompanyDataModel) -> Optional[CompanyDataModel]:
        """Get single company matching filter."""
        results = self.get(filter_model)
        return results[0] if results else None
    
    def get_by_id(self, id: int) -> Optional[CompanyDataModel]:
        """Get company by ID."""
        return self._companies.get(id)
    
    def count(self, filter_model: Optional[CompanyDataModel] = None) -> int:
    ...
    def update(self, filter_model: CompanyDataModel, update_data: CompanyDataModel) -> int:
...
if __name__ == "__main__":
    ...
    # Display sample data
    print(f"Mock repository initialized with {mock_repo.get_company_count()} companies:")
