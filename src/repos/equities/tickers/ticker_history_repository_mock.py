...
class TickerHistoryRepositoryMock:
    ...
    def __init__(self, seed: int = 12345):
        ...
        self._next_id = 1
        self._symbol_histories: Dict[str, List[int]] = {}  # symbol -> list of history_ids
        self._company_histories: Dict[int, List[int]] = {}  # company_id -> list of history_ids
    ...
    def _initialize_sample_data(self) -> None:
        ...
        sample_histories = [
            ...
            {'symbol': 'AAPL', 'company_id': 1, 'valid_from': today - timedelta(days=365*10), 'valid_to': None, 'active': True},
            {'symbol': 'MSFT', 'company_id': 2, 'valid_from': today - timedelta(days=365*15), 'valid_to': None, 'active': True},
            {'symbol': 'AMZN', 'company_id': 3, 'valid_from': today - timedelta(days=365*8), 'valid_to': None, 'active': True},
            
            # Historical ticker changes
            {'symbol': 'FB', 'company_id': 6, 'valid_from': today - timedelta(days=365*5), 'valid_to': today - timedelta(days=365), 'active': False},
            {'symbol': 'META', 'company_id': 6, 'valid_from': today - timedelta(days=365), 'valid_to': None, 'active': True},
            
            # Multiple tickers for same company
            {'symbol': 'GOOGL', 'company_id': 4, 'valid_from': today - timedelta(days=365*3), 'valid_to': None, 'active': True},
            {'symbol': 'GOOG', 'company_id': 4, 'valid_from': today - timedelta(days=365*3), 'valid_to': None, 'active': True},
            
            # Some inactive historical entries
            {'symbol': 'NFLX', 'company_id': 7, 'valid_from': today - timedelta(days=365*6), 'valid_to': today - timedelta(days=30), 'active': False},
            {'symbol': 'NFLX', 'company_id': 7, 'valid_from': today - timedelta(days=30), 'valid_to': None, 'active': True},
        ]
    ...
    def _create_fake_ticker_history(self, **overrides) -> TickerHistoryDataModel:
        ...
        default_data = {
            ...
            'valid_to': None if self.fake.boolean(chance_of_getting_true=70) else today - timedelta(days=self.fake.random_int(1, 30)),
    ...
    def _is_currently_active(self, history: TickerHistoryDataModel) -> bool:
        ...
        today = date.today()
        return (history.active and
                history.valid_from <= today and
                (history.valid_to is None or history.valid_to >= today))
    
    # Base repository methods
    def get(self, filter_model: Optional[TickerHistoryDataModel] = None, options = None) -> List[TickerHistoryDataModel]:
        ...
        for history in histories:
            ...
            if (filter_model.company_id is not None and filter_model.company_id != 0
                ...
                match = False
            if filter_model.active is not None and history.active != filter_model.active:
                match = False
        ...
        if options and hasattr(options, 'order_by') and options.order_by == "valid_from":
            filtered.sort(key=lambda h: h.valid_from)
        
        return filtered
    
    def get_one(self, filter_model: TickerHistoryDataModel) -> Optional[TickerHistoryDataModel]:
        """Get single ticker history matching filter."""
        results = self.get(filter_model)
        return results[0] if results else None
    
    def get_by_id(self, id: int) -> Optional[TickerHistoryDataModel]:
        """Get ticker history by ID."""
        return self._ticker_histories.get(id)
    
    def count(self, filter_model: Optional[TickerHistoryDataModel] = None) -> int:
    ...
    def insert(self, data_model: TickerHistoryDataModel) -> TickerHistoryDataModel:
        ...
        logger.info(f"Mock: Inserted ticker history {data_model.symbol} with ID {data_model.id}")
        return data_model
    ...
    def update(self, filter_model: TickerHistoryDataModel, update_data: TickerHistoryDataModel) -> int:
    ...
    def get_active_ticker_history_symbols(self) -> Set[str]:
        ...
        logger.info(f"Mock: Retrieved {len(active_symbols)} active ticker history symbols")
    ...
    def get_ticker_history_for_company(self, company_id: int) -> List[TickerHistoryDataModel]:
        ...
        histories = [self._ticker_histories[hid] for hid in history_ids if hid in self._ticker_histories]
        
        # Sort by valid_from date
        histories.sort(key=lambda h: h.valid_from)
        
        logger.info(f"Mock: Retrieved {len(histories)} ticker histories for company {company_id}")
        return histories
    ...
    def get_ticker_history_by_symbol(self, symbol: str) -> List[TickerHistoryDataModel]:
        ...
        logger.info(f"Mock: Retrieved {len(histories)} ticker histories for symbol {symbol}")
        return histories
    ...
    def get_active_ticker_histories(self) -> List[TickerHistoryDataModel]:
        ...
        return [history for history in self._ticker_histories.values() 
                if self._is_currently_active(history)]
    ...
    def deactivate_ticker_history(self, symbol: str, company_id: int, 
        ...
        matching_histories = []
        for history in self._ticker_histories.values():
            if (history.symbol == symbol and 
                history.company_id == company_id and 
                history.active and 
                (history.valid_to is None or history.valid_to >= end_date)):
    ...
    def add_fake_ticker_histories(self, count: int = 5, 
                                company_id: Optional[int] = None,
                                symbol: Optional[str] = None) -> List[TickerHistoryDataModel]:
    ...
    def get_symbols_with_multiple_histories(self) -> List[str]:
        ...
        return [symbol for symbol, history_ids in self._symbol_histories.items() 
    ...
    def get_history_for_date_range(self, start_date: date, end_date: date) -> List[TickerHistoryDataModel]:
...
def create_ticker_history_repository_mock(seed: int = 12345) -> TickerHistoryRepositoryMock:
...
if __name__ == "__main__":
    ...
    # Display sample data
    print(f"Mock repository initialized with {mock_repo.get_ticker_history_count()} ticker histories")
    ...
    for symbol in multi_history_symbols:
        ...
        for history in histories:
            ...
            valid_to_str = history.valid_to.strftime("%Y-%m-%d") if history.valid_to else "Present"
            print(f"    {history.valid_from.strftime('%Y-%m-%d')} to {valid_to_str} [{status}]")
