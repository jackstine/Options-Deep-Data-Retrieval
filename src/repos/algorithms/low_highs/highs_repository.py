...
from src.database.equities.tables.algorithms.highs import High as HighDBModel
...
class HighsRepository(
    ...
    def from_db_model(db_model: HighDBModel) -> HighDataModel:
        ...
        """
        from src.database.equities.tables.algorithms.highs import PRICE_MULTIPLIER
    ...
    def get_by_ticker_history_id(self, ticker_history_id: int) -> list[HighDataModel]:
        """Get all high patterns for a specific ticker history.

        Args:
            ticker_history_id: The ticker history ID to filter by

        Returns:
            List of High patterns for the specified ticker history
        """
        try:
            with self._SessionLocal() as session:
                stmt = select(HighDBModel).where(
            ...
            logger.error(
                f"Database error retrieving highs for ticker_history_id={ticker_history_id}: {e}"
            )
            raise
